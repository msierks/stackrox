...
function generateURL(workflowState: WorkflowState) {
    ...
    if (!(workflowState.search && workflowState.sort && workflowState.paging)) {
        // TODO This check probably indicates a logic error that needs to be cleaned up.
        // These properties can be set to null or undefined in the WorkflowState constructor.
        // Should they be?
        //
        // If so, can we provide meaningful default values here instead of an error?
        // If not, can we type this in a way that ensures these properties are always defined?
        throw new Error('A required WorkflowState property was null or undefined here');
    }
