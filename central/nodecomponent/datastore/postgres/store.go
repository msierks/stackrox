...
const (
	...
	existsStmt = "SELECT EXISTS(SELECT 1 FROM node_components WHERE Id = $1 AND Name = $2 AND Version = $3 AND OperatingSystem = $4)"

	getStmt     = "SELECT serialized FROM node_components WHERE Id = $1 AND Name = $2 AND Version = $3 AND OperatingSystem = $4"
	deleteStmt  = "DELETE FROM node_components WHERE Id = $1 AND Name = $2 AND Version = $3 AND OperatingSystem = $4"
	walkStmt    = "SELECT serialized FROM node_components"
	getIDsStmt  = "SELECT Id FROM node_components"
	getManyStmt = "SELECT serialized FROM node_components WHERE Id = ANY($1::text[])"

	deleteManyStmt = "DELETE FROM node_components WHERE Id = ANY($1::text[])"
...
func (s *storeImpl) copyFromNodeComponents(ctx context.Context, tx pgx.Tx, objs ...*storage.ImageComponent) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			_, err = tx.CopyFrom(ctx, pgx.Identifier{"node_components"}, copyCols, pgx.CopyFromRows(inputRows))
...
func (s *storeImpl) copyFrom(ctx context.Context, objs ...*storage.ImageComponent) error {
	...
	if err := s.copyFromNodeComponents(ctx, tx, objs...); err != nil {
...
func dropTableNodeComponents(ctx context.Context, db *pgxpool.Pool) {
	_, _ = db.Exec(ctx, "DROP TABLE IF EXISTS node_components CASCADE")

}

func Destroy(ctx context.Context, db *pgxpool.Pool) {
	dropTableNodeComponents(ctx, db)
}
